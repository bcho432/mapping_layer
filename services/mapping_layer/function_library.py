...
# `label` is the one reduction whose input is NOT numeric. Every other function
# here folds numbers; this one folds categories and hands back an integer class
# id, so the frame stays a numeric matrix and any engine can consume it without
# being told what the columns mean. The id-to-name mapping travels in the
# frame's metadata rather than in a column, for the same reason.
